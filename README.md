...
Once the `conda-forge` channel has been enabled, `libmujoco, mujoco, mujoco-mjx, mujoco-python, mujoco-samples` can be installed with `conda`:

```
conda install libmujoco mujoco mujoco-mjx mujoco-python mujoco-samples
```

or with `mamba`:

```
mamba install libmujoco mujoco mujoco-mjx mujoco-python mujoco-samples
```
